...
class Bill extends Admin_Controller
{
    ...
    public function add_discharge()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'ex_finding'          => $this->input->post('ex_finding'),
                'system_finding'      => $this->input->post('system_finding'),
                'adminssion_time_complaints' => $this->input->post('adminssion_time_complaints'),
                'treatment_given'     => $this->input->post('treatment_given'),
                'advice_follow_up'    => $this->input->post('advice_follow_up'),
    ...
    public function print_dischargecard(){
        ...
        if ($query->num_rows() > 0) {
            $ipd_details = $query->result_array(); // Fetch data as an array
            
        } 
        $data['print_details']  = $print_details;
        $data['case_id']        = $case_id;
        $data['ipd_details']        = $ipd_details;
        // print_r($data['ipd_details']);
        // die;
        $data['result']         = $patient;
        $type                   = $this->input->post('module_type');
        $data['discharge_card'] = $this->patient_model->get_dischargeCard(array('case_reference_id' => $case_id));
        // print_r($data['discharge_card']);
        // die;
        $data['deathrecord']    = array();
        $data['patient_id']     = $patient['patient_id'];
        $data['guardian_name']  = $patient['guardian_name'];
